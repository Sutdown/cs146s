...
# 封装 OpenAI API 调用，使用 gpt-4-turbo 模型
def execute_llm_call(conversation: list[ChatCompletionMessageParam]) -> str:
    ...
    while True:
        ...
        while True:
            ...
            for name, args in tool_invocations:
                ...
                print(name, args)
                if name == "read_file":
                    resp = read_file_tool(str(args.get("filename", ".")))
                elif name == "list_files":
                    resp = list_files_tool(str(args.get("path", ".")))
                elif name == "edit_file":
                    resp = edit_file_tool(
                        str(args.get("path", ".")), 
